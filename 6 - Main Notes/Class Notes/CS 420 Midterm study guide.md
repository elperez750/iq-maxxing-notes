...
https://dev.to/hari_saravanan_7ddd3f0bbf/database-normalization-in-sql-1nf2nf-and-3nf-explained-student-course-case-study-3b2c
...
###### DBA is Database Admin and I'm guessing that the end user would just simply be the person who is indirectly accessing the database.
...
Some examples for the super key and the primary key:
For relation R(sid, name, email) where sid is unique and email is also unique:

- A super key here would be all of the attributes, so (sid, name, email). The reasoning here is that they all define a unique entity, but it is not the minimal value to do that, since we can uniquely identify a tuple with either sid or email

- A candidate key would be the sid or the email. It would depend on which one we use for the primary key.
...
Relation R(student_id, course_id, student_name) with primary key(student_id, course_id).

FDs:
- (student_id, course_id) -> student_name
- student_id ->student_name

Is the answer in 2NF? Briefly explain


###### The answer would not be in 2NF, since the student_name depends on the student_id alone, which is part of the composite key
...
- A dependent must belong to exactly one employee

Scenario 2
- A department controls one to many projects
- Each project is controlled by only one department
